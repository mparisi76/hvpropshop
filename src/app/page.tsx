...
export default async function HomePage({ searchParams }: { searchParams: Promise<{ category?: string, search?: string }> }) {
  ...
  for (const parent of flatParents) {
    ...
    const child = Object.values(parent.children).find(c => c.slug === selectedSlug);
    if (child) {
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6">
      <div className="flex flex-col lg:flex-row lg:gap-10">
        ...
        <main className="flex-1 min-w-0">
          ...
          */}
          <div className="sticky top-16 z-20 bg-white -mx-4 px-4 md:mx-0 md:px-0 border-b border-slate-100 lg:border-none">
            <Breadcrumbs 
          ...
          {/* CONTENT FEED */}
          <div className="pt-4 pb-10 md:pt-10">
              ...
              <div className="text-center py-24 bg-slate-50 rounded-4xl border-2 border-dashed border-slate-200 flex flex-col items-center">
                ...
                <p className="text-slate-500 text-sm max-w-sm px-6">
                  {searchQuery ? (
                    <>
                      ...
                      {breadcrumbCurrent && <> in <span className="font-bold text-slate-900">{breadcrumbCurrent.name}</span></>}
                    </>
                  ) : (
                    <>
                      There are currently no items available in the <span className="font-bold text-slate-900 uppercase tracking-tighter">{breadcrumbCurrent?.name || 'selected'}</span> category.
                    </>
                  )}
                </p>
                ...
                <div className="flex flex-col gap-4 mt-8">
                  {selectedSlug && searchQuery && (
                    <Link href={`/?search=${searchQuery}`} className="text-[10px] font-black uppercase tracking-widest text-blue-600 hover:text-blue-700">
                      Search entire database instead
                    </Link>
                  )}
                  <Link href="/" className="text-[10px] font-black uppercase tracking-widest bg-slate-900 text-white px-8 py-3 rounded-full hover:bg-blue-600 transition-colors">
                    Clear all filters
                  </Link>
                </div>
              </div>
